...
class AccdailyBal extends \yii\db\ActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'Debit_tur' => Yii::t('app', 'Debit Tur'),
            'Cedit_tur' => Yii::t('app', 'Credit Tur'),
        ];
    }
    ...
    {
        $balance=AccdailyBal::find()->where(['account'=>$id,'value_date'=>SystemDate::getCurrentDate()])->one();
        if($balance!=null){
    ...
    public static function updateAccountBalance($account,$amount,$drcr)
    {
        ...
        $balance=AccdailyBal::getBalance($account);
            ...
            if($drcr=='C') {
                if($balance!=null) {
                    if($balance->value_date==SystemDate::getCurrentDate()) {
                        $newbalance = $balance->available_balance + $amount;
                        $cr_tur = $balance->Cedit_tur + $amount;
                        AccdailyBal::updateAll(['available_balance' => $newbalance, 'Cedit_tur' => $cr_tur], ['id' => $balance->id]);
                    }
                    else{
                        $customerbalance=new AccdailyBal();
                        $customerbalance->branch_code=Account::getBranchCode($account);
                        $customerbalance->value_date=SystemDate::getCurrentDate();
                ...
                if ($balance != null) {
                    if($balance->value_date==SystemDate::getCurrentDate()) {
                        ...
                        AccdailyBal::updateAll(['available_balance' => $newbalance, 'Debit_tur' => $dr_tur], ['id' => $balance->id]);
                    }
                    else{
                        $customerbalance=new AccdailyBal();
                        $customerbalance->branch_code=Account::getBranchCode($account);
